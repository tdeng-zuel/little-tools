...
def plotPie(col_name, theme, rosetype=None):
    ...
    Pie(init_opts=opts.InitOpts(theme=theme, width='1150px', height='750px', bg_color=background_color)).add(
    ...
    ).set_series_opts(label_opts=opts.LabelOpts(formatter="{b}: {d}%", font_size=24)).render(file_name + '.html')
...
def plotBar(col_name):
    ...
    grid = Grid(init_opts=opts.InitOpts(theme=ThemeType.LIGHT, width='900px', height='700px', bg_color=background_color))
    bar = Bar().add_xaxis(counts.index.tolist()). \
        ...
        label_opts=opts.LabelOpts(position="right", font_size=17)).set_global_opts(
        toolbox_opts=opts.ToolboxOpts(),
        xaxis_opts=opts.AxisOpts(axislabel_opts=opts.LabelOpts(font_size=17)),
        yaxis_opts=opts.AxisOpts(axislabel_opts=opts.LabelOpts(font_size=22)))
...
plt.savefig('阅读介质.jpg', dpi=150, bbox_inches='tight')
plt.clf()
...
Bar(init_opts=opts.InitOpts(theme=ThemeType.LIGHT, bg_color='white')). \
    ...
    set_global_opts(
    xaxis_opts=opts.AxisOpts(axislabel_opts=opts.LabelOpts(interval=0, margin=10, font_size=17)),
    yaxis_opts=opts.AxisOpts(axislabel_opts=opts.LabelOpts(font_size=22)),
    toolbox_opts=opts.ToolboxOpts()). \
    render("第5题.html")
...
plt.yticks(ticks=range(-5, 0), labels=range(5, 0, -1))
plt.ylabel('\n'.join(['顺', '序']), rotation='horizontal', verticalalignment='bottom', horizontalalignment='right')
plt.savefig('渠道.jpg', dpi=150, bbox_inches='tight')
plt.clf()
...
rect = ax.patch
rect.set_facecolor(background_color)
sns.scatterplot(x='类别', y='顺序', hue='类别', palette='Set2', size='人数', sizes=(0, 1000), data=scatter_table, legend=False, ax=ax)
plt.grid(b=True, linewidth=0.5)
plt.ylim([-7.5, -0.5])
plt.yticks(ticks=range(-7, 0), labels=range(7, 0, -1))
plt.ylabel('\n'.join(['顺', '序']), rotation='horizontal', verticalalignment='bottom', horizontalalignment='right')
plt.savefig('类别.jpg', dpi=150, bbox_inches='tight')
plt.clf()
...
Sunburst(init_opts=opts.InitOpts(width="1000px", height="750px", bg_color=background_color)).add(
        "",
        data_pair=data,
